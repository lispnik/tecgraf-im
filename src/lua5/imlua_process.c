...
/*****************************************************************************\
 Local Utilities
\*****************************************************************************/
static void imlua_errorcomplex(lua_State *L, int index)
{
  luaL_argerror(L, index, "image data type can NOT be complex");
}
...
static int imluaCalcHistogram (lua_State *L)
{
  ...
  unsigned long *histo;
  imImage* src_image = imlua_checkimage(L, 1);
  int plane = (int)luaL_checkinteger(L, 2);
...
static int imluaProcessRemoveByArea (lua_State *L)
{
  ...
  imImage* dst_image = imlua_checkimage(L, 2);
  int connect = (int)luaL_checkinteger(L, 3);
  int start_size = (int)luaL_checkinteger(L, 4);
  int end_size = (int)luaL_checkinteger(L, 5);
...
static int imluaProcessInsert (lua_State *L)
{
  ...
  int xmin = (int)luaL_checkinteger(L, 4);
...
static int imluaProcessGrayMorphErode (lua_State *L)
{
  imImage *src_image = imlua_checkimage(L, 1);
  imImage *dst_image = imlua_checkimage(L, 2);
  int kernel_size = (int)luaL_checkinteger(L, 3);
...
static int imluaProcessBinMorphConvolve (lua_State *L)
{
  ...
  int iter = (int)luaL_checkinteger(L, 5);
...
static int imluaProcessBinMorphErode (lua_State *L)
{
  ...
  imImage *dst_image = imlua_checkimage(L, 2);
  int kernel_size = (int)luaL_checkinteger(L, 3);
  int iter = (int)luaL_checkinteger(L, 4);
...
static int imluaProcessArithmeticOp (lua_State *L)
{
  ...
  imImage *dst_image = imlua_checkimage(L, 3);
  int op = (int)luaL_checkinteger(L, 4);
...
static int imluaProcessBackSub(lua_State *L)
{
  imImage *src_image1 = imlua_checkimage(L, 1);
  imImage *src_image2 = imlua_checkimage(L, 2);
  imImage *dst_image = imlua_checkimage(L, 3);
  ...
  imProcessBackSub(src_image1, src_image2, dst_image, luaL_checknumber(L, 4), lua_toboolean(L, 5));
  return 0;
}
...
static int imluaProcessPseudoColor(lua_State *L)
{
  ...
  imlua_checkcolorspace(L, 2, dst_image, IM_RGB);
  imlua_checkdatatype(L, 2, dst_image, IM_BYTE);

  imProcessPseudoColor(src_image, dst_image);
  return 0;
}
...
static int imluaProcessSelectHue(lua_State *L)
{
  imImage *src_image = imlua_checkimage(L, 1);
  imImage *dst_image = imlua_checkimage(L, 2);
  ...
  imProcessSelectHue(src_image, dst_image, luaL_checknumber(L, 3), luaL_checknumber(L, 4));
  return 0;
}
...
{
  imImage *image = imlua_checkimage(L, 1);
  int int_radius = (int)luaL_checkinteger(L, 2);
  int ext_radius = (int)luaL_checkinteger(L, 3);
...
static int imluaProcessRenderRamp (lua_State *L)
{
  ...
  int dir = (int)luaL_checkinteger(L, 4);
...
{
  imImage *image = imlua_checkimage(L, 1);
  int x_space = (int)luaL_checkinteger(L, 2);
  int y_space = (int)luaL_checkinteger(L, 3);
...
static int imluaProcessRangeContrastThreshold (lua_State *L)
{
  ...
  imImage *dst_image = imlua_checkimage(L, 2);
  int kernel_size = (int)luaL_checkinteger(L, 3);
  int min_range = (int)luaL_checkinteger(L, 4);
...
static int imluaProcessLocalMaxThreshold (lua_State *L)
{
  ...
  imImage *dst_image = imlua_checkimage(L, 2);
  int kernel_size = (int)luaL_checkinteger(L, 3);
  int min_thres = (int)luaL_checkinteger(L, 4);
...
static int imluaProcessThresholdByDiff (lua_State *L)
{
  ...
  imlua_checknotcomplex(L, 1, src_image1);
  imlua_match(L, src_image1, src_image2);
  imlua_checkcolorspace(L, 2, dst_image, IM_BINARY);
...
static int imluaProcessHysteresisThreshold (lua_State *L)
{
  imImage *src_image = imlua_checkimage(L, 1);
  imImage *dst_image = imlua_checkimage(L, 2);
  int low_thres = (int)luaL_checkinteger(L, 3);
  int high_thres = (int)luaL_checkinteger(L, 4);
...
static int imluaProcessPixelate (lua_State *L)
{
  imImage *src_image = imlua_checkimage(L, 1);
  imImage *dst_image = imlua_checkimage(L, 2);
  int box_size = (int)luaL_checkinteger(L, 3);
...
static int imluaProcessAbnormalHyperionCorrection(lua_State *L)
{
  ...
  imProcessAbnormalHyperionCorrection(src_image, dst_image, (int)luaL_checkinteger(L, 3), (int)luaL_checkinteger(L, 4), image_abnormal);
  return 0;
}
...
{
  lua_pushinteger(L, imProcessOpenMPSetMinCount((int)luaL_checkinteger(L, 1)));
  return 1;
}

static int imlua_ProcessOpenMPSetNumThreads(lua_State *L)
{
  lua_pushinteger(L, imProcessOpenMPSetNumThreads((int)luaL_checkinteger(L, 1)));
  return 1;
}
